...
from dppy.utils import check_random_state, eval_kernel_diagonal, kernel_polynomial
...
class TestBless(unittest.TestCase):
    ...
    def test_reduce_lambda(self):
        ...
        rls_estimates = estimate_rls_bless(dict_approx, X_data, kernel_polynomial, lam)
        np.testing.assert_allclose(rls_estimates, rls_exact, rtol=0.5)

        dict_reduced = reduce_lambda(
            X_data, kernel_polynomial, dict_approx, lam_new, rng
        )

        rls_estimates_reduced = estimate_rls_bless(
            dict_reduced, X_data, kernel_polynomial, lam_new
        )
    ...
    def test_bless(self):
        ...
        rls_estimates = estimate_rls_bless(dict_reduced, X_data, kernel_polynomial, lam)
