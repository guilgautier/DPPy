...
class TestUtils(unittest.TestCase):
                ...
                for test_inner1D in ["inner_prod", "sq_norm"]:
                    with self.subTest(test_inner1D=test_inner1D):
                        ...
                        if test_inner1D == "inner_prod":
                            ...
                            self.assertTrue(
                                np.allclose(
                                    utils.inner1d(X, Y, axis=ax), (X * Y).sum(axis=ax)
                                )
                            )
                        ...
                        if test_inner1D == "sq_norm":
                            ...
                            self.assertTrue(
                                np.allclose(
                                    utils.inner1d(X, axis=ax), (X ** 2).sum(axis=ax)
                                )
                            )
    ...
    def test_symmetric(self):
        ...
        list_of_inputs = [(True, None), (False, X), (True, X.T + X), (True, X.T.dot(X))]
    ...
    def test_is_projection(self):
        ...
        for idx, (flag, _input) in enumerate(list_of_inputs):
            with self.subTest(index=idx, is_projection=flag):
                ...
                if flag:
                    self.assertTrue(utils.is_projection(_input) is _input)
                else:
                    with self.assertRaises(ValueError) as context:
    ...
    def test_is_orthonormal_columns(self):
        ...
        list_of_inputs = [(True, None), (False, X), (True, e_vecs)]

        for idx, (flag, _input) in enumerate(list_of_inputs):
            with self.subTest(index=idx, is_orthonormal_columns=flag):

                if flag:
                    self.assertTrue(utils.is_orthonormal_columns(_input) is _input)
                else:
                    with self.assertRaises(ValueError) as context:
    ...
    def test_is_equal_to_O_or_1(self):
        ...
        for idx, (flag, _input) in enumerate(list_of_inputs):
            with self.subTest(index=idx, is_equal_to_O_or_1=flag):
                ...
                if flag:
                    self.assertTrue(utils.is_equal_to_O_or_1(_input, tol) is _input)
                else:
                    with self.assertRaises(ValueError) as context:
