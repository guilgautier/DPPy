...
class UST:
    r"""The Uniform Spanning Tree measure corresponds to the uniform measure on spanning trees (UST) of an undirected connected graph :math:`G`.

    This is a DPP the edges of the graph :math:`G` with correlation kernel the projection kernel onto the span of the rows of the oriented vertex-edge incidence matrix :math:`\text{Inc}` of :math:`G`.
    ...
    @property
    def kernel(self):
        r"""Compute the orthogonal projection kernel :math:`\mathbf{K} = \text{Inc}^+ \text{Inc}` onto the span of the rows of the oriented vertex-edge incidence matrix :math:`\text{Inc}` of size :math:`|V| \times |E|`.

        For a connected graph, :math:`\text{Inc}` has rank :math:`|V|-1` and any row can be discarded to get a basis of the row space. If we note :math:`A` the amputated version of :math:`\text{Inc}`, then :math:`\text{Inc}^+ = A^{\top}[AA^{\top}]^{-1}`.
    ...
    def plot_kernel(self, ax=None):
        ...
        return self._dpp.plot_kernel(kernel_type="correlation", ax=ax)
