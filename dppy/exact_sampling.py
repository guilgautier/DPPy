...
def projection_dpp_sampler_Schur(K, k=None):
    ...
    for it in range(k):
        ...
        schur_comp[rem_set] = K_diag[rem_set] - inner1d(K[np.ix_(rem_set,Y)], K[np.ix_(rem_set,Y)].dot(K_inv))
...
def dpp_sampler_eig_Cholesky(eig_vals, eig_vecs):
    ...
    for it in range(k):
        ...
        c[rem_set, it] = (V[rem_set,:].dot(V[j,:]) - c[rem_set,:it].dot(c[j,:it]))/np.sqrt(norms_2[j])
...
def dpp_sampler_KuTa12(eig_vals, eig_vecs):
    ...
    for it in range(1,k):
        ...
        # V_:j is set to 0 so we delete it and we can
