...
from bisect import bisect_right # for __RSK 
...
class UST:
	...
	def __init__(self, graph):
		...
		self.sampling_mode = 'Wilson' # sampling mode
...
class Descent: 
	...
	def plot(self, title=''):
		...
		len_sam = len(sampl)
		ax.scatter(sampl, np.zeros(len_sam), color='blue', s=20, label=self.name)
	...
	def plot_vs_bernoullis(self, title=''):
		...
		ax.scatter(sampl, np.ones(len_sam), color='b', s=20, label=self.name)
		ax.scatter(bern, -np.ones(len_ber), color='r', s=20, label='Bernoullis')

		str_title = r'Realization of the {} process vs independent Bernoulli variables with parameter p={}'.format(self.name, self.bernoulli_param)
		plt.title(title if title else str_title)
